...
rrBLUP <- function(genoModified, phenotype, evaluation=F, outFile, folds) {
  if(evaluation) {
    ...
    for(i in 1:length(folds)) {
      ...
      fit <-mixed.solve(phenotype[train], Z=genoModified[train,],K=NULL, SE=F,return.Hinv = F)
      pred <- genoModified[test,] %*% as.matrix(fit$u)
      prediction <- c(prediction, list(pred[,1]+fit$beta))
    }
    saveRDS(prediction, file=paste(outFile, ".rds", sep = ""))
  } else {
    model <-mixed.solve(phenotype, Z=genoModified,K=NULL, SE=F,return.Hinv = F)
...
# run !
rrBLUP(genoModified = t(genotype), phenotype = phenotype, evaluation = evaluation,outFile = out, folds = folds)
