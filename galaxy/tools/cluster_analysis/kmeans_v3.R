...
option_list = list(
  ...
  make_option(c("-n", "--nstart"), type="integer", default=1, 
              help="the starting N random sets", metavar="nstart"),
...
require(factoextra)
result <- kmeans(data2, centers=opt$kcluster, algorithm=opt$algorithm,nstart=opt$nstart, iter.max= opt$iter)
...
write.table(data3, file = opt$clusterfile, row.names = FALSE,sep="\t", quote=FALSE,eol="\n",na="NA")
